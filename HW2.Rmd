...
sample_and_fit = function(sample_size, gauss_num){
  ...
  out1 = fit_em(my_sample, weights, mu, sigma, n_iter = 4001)
  print(out1)
...
```{r}
sample_and_fit(sample_size = 25, gauss_num = 8)
```
```{r}
sample_and_fit(sample_size = 2000, gauss_num = 8)
```
```{r}
k_max = 8
p_list = list()
...
for(k in 1:k_max){
  par = initialize_parameters(XX = XX, n = n, k = k)
...
AIC = function(y, kmax, n_iter, p, mu, sigma) {
  ...
  for (k in 1:kmax){
    ...
    out.opt = fit_em(y, p, mu, sigma, n_iter = 20, plot_flag = F)
...
BIC = function(y, kmax, n_iter) {
  ...
  for (k in 1:kmax){
    ...
    out.opt = fit_em(y, p, mu, sigma, n_iter = n_iter, plot_flag = F)
...
training.model = function(y_train, y_test, kmax, n_iter) {
  ...
  for (k in 1:kmax){
    ...
    out.opt = fit_em(y_train, p, mu, sigma, n_iter = n_iter, plot_flag = F) # get the estimated parameters
...
k.fold.cross.validation = function(y, k_folds, kmax, n_iter) {
  ...
  for (k in 1:kmax) { # train the model kmax iterations
    for (x in 1:k_folds) { # train each fold k times
      ...
      out.opt = fit_em(train.set, p, mu, sigma, n_iter = n_iter, plot_flag = F)
